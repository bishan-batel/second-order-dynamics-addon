...
/// </summary>
[GlobalClass, Tool, Icon("res://addons/SecondOrderDynamics/Icons/SodRemoteTransform3D.svg")]
public partial class SodRemoteTransform3D : Node3D {
  #region Exports

  /// <summary>
  /// The node that this will be interpolating to. Note that this is the opposite of how RemoteTransform3D works.
  /// </summary>
  [Export(PropertyHint.None, "Target Node to pull the target transform from")]
  public Node3D? Following {
    set {
      _following = value;
      #if TOOLS
  ...
  public bool RunInEditor {
    set {
      ...
      if (Engine.IsEditorHint() && !RunInEditor && TryGetFollowing(out Node3D following)) {
  ...
  /// </summary>
  // [Export]
  public bool DrawGizmos { set; get; } = true;
  ...
  public SodRemoteTransform3D() {
    ...
    EditorDescription =
      ...
      which can be configured with the 'Params' property.
      """;
  }

  /// <summary>
  /// Tries to find the node being followed, will fail if <see cref="Following"/> is an invalid path or said node is not a Node3D.
  /// </summary>
  /// <param name="following"></param>
  /// <returns>Success, do not read the out parameter if this is false.</returns>
  public bool TryGetFollowing(out Node3D following) {
  ...
  void _update(float delta) {
    ...
    if (!TryGetFollowing(out Node3D following)) return;

    if (UpdatePosition && _updatePosition(delta, out Vector3 position, UseLocalCoordinates ? following.Position : following.GlobalPosition)) {
      if (UseLocalCoordinates) Position = position;
      else GlobalPosition = position;
    }

    if (UpdateRotation && _updateRotation(delta, out Quaternion rotation, (UseLocalCoordinates ? following.Basis : following.GlobalBasis).GetRotationQuaternion())) {
      if (UseLocalCoordinates) Rotation = rotation.GetEuler();
      else GlobalRotation = rotation.GetEuler();
    }

    if (UpdateScale && _updateScale(delta, out Vector3 scale, UseLocalCoordinates ? following.Scale : following.GlobalBasis.Scale)) {
      if (UseLocalCoordinates) Scale = scale;
      else Scale = scale / GlobalBasis.Scale;
    }
  }

  bool _updatePosition(float delta, out Vector3 position, Vector3 target) {
    if (PositionParams is null) {
      position = default;
      return false;
    }

    _sodPosition ??= new SodVector3(PositionParams, target);

    position = _sodPosition.Update(delta, target);
    return true;
  }

  bool _updateRotation(float delta, out Quaternion rotation, Quaternion target) {
    if (RotationParams is null) {
      rotation = Quaternion.Identity;
      return false;
    }

    _sodRotation ??= new SodQuaternion(RotationParams, target);
  ...
  bool _updateScale(float delta, out Vector3 scale, Vector3 target) {
    if (ScaleParams is null) {
      scale = Vector3.One;
      return false;
    }

    _sodScale ??= new SodVector3(ScaleParams, target);
  ...
  public override void _ValidateProperty(Dictionary property) {
    ...
    if (UpdatePosition && name == Node3D.PropertyName.Position) {
      property["usage"] = (int)PropertyUsageFlags.NoEditor;
      return;
    }

    if (UpdateRotation && name == Node3D.PropertyName.Rotation) {
      property["usage"] = (int)PropertyUsageFlags.NoEditor;
      return;
    }

    if (UpdateScale && name == Node3D.PropertyName.Scale) {
      property["usage"] = (int)PropertyUsageFlags.NoEditor;
      return;
    }
  ...
  public override string[] _GetConfigurationWarnings() {
    ...
    if (!TryGetFollowing(out Node3D _)) {
  ...
  void _forceTransformToFollowing(Node3D following) {
    if (UpdatePosition) {
